...
namespace Compiler
{
    ...
    static constexpr CConstantInfoSpawnFunctionTable ConstantInfoSpawnFunctions = GetConstantInfoSpawnFunctions();

    CConstantInfo* CConstantInfo::NewConstantInfo(const EConstantPoolInfoTag ConstantPoolInfoTag)
    {
        using CTagSizeType = std::underlying_type_t<EConstantPoolInfoTag>;

        const CTagSizeType Index = static_cast<CTagSizeType>(ConstantPoolInfoTag);
        ASSERT_MSG
        (
            (Index >= static_cast<CTagSizeType>(0)) && (Index < static_cast<CTagSizeType>(ConstantInfoSpawnFunctions.size())),
            "Invalid tag index: %d", static_cast<int>(Index)
        )

        const CConstantInfoSpawnFunction ConstantInfoSpawnFunction = ConstantInfoSpawnFunctions[Index];
        ASSERT_MSG(ConstantInfoSpawnFunction != nullptr, "Invalid tag index: %d", static_cast<int>(Index))
